  ...
  const html = `
    ...
    <div class="dashboard-grid">
      ...
      <div class="dashboard-section">
        ...
        <div class="dashboard-section-body">
          ${tareasPendientesList.length > 0 ? `<div style="display:flex;flex-direction:column;gap:8px">${tareasPendientesList.map(t => `<div style="display:flex;align-items:center;gap:12px;padding:8px 0;border-bottom:1px solid var(--border-subtle)"><input type="checkbox" style="accent-color:var(--oc-black)" ${lastEtapa && getTaskEtapaId(t) === lastEtapa.id ? 'checked' : ''}><span style="flex:1;font-size:var(--text-sm)">${t.titulo}</span><span class="badge ${t.prioridad === 'alta' ? 'badge-error' : t.prioridad === 'media' ? 'badge-warning' : 'badge-neutral'}">${t.prioridad}</span></div>`).join('')}</div>` : '<div style="padding:var(--space-lg);text-align:center;color:var(--text-tertiary);font-size:var(--text-sm)">Sin tareas pendientes</div>'}
        </div>
      </div>
    </div>
  `;
